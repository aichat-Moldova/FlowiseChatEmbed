...
export const BotBubble = (props: Props) => {
  ...
  const [showFeedbackContentDialog, setShowFeedbackContentModal] = createSignal(
    false
  );
  ...
  const renderArtifacts = (item: Partial<FileUpload>) => {
    return (
      <>
            ...
            <img
              ...
              src={(() => {
                const isFileStorage =
                  ...
                  item.data.startsWith("FILE-STORAGE::");
                return isFileStorage
                  ? `${props.apiHost}/api/v1/get-upload-file?chatflowId=${
                      props.chatflowid
                    }&chatId=${
                      props.chatId
                    }&fileName=${(item.data as string).replace(
                      "FILE-STORAGE::",
                      ""
                    )}`
                  : (item.data as string);
        ...
        <Show
          when={
            item.type !== "png" && item.type !== "jpeg" && item.type !== "html"
          }
        >
          <span
      ...
      >
        ...
        <div class="flex flex-col justify-start">
          {props.showAgentMessages && props.message.agentReasoning && (
            <details
              ref={botDetailsEl}
              class="mb-2 px-4 py-2 ml-2 chatbot-host-bubble rounded-[6px]"
            >
              ...
              <Show when={props.message.dateTime}>
                <div class="text-sm text-gray-500 ml-2">
                  {formatDateTime(
                    ...
                    props?.dateTimeToggle?.date,
                    props?.dateTimeToggle?.time
                  )}
                </div>
              </Show>
            </div>
